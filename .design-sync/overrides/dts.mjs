...
// FORK: two real Yappr page sections trip the generic heuristics —
// `FAQ` looks like a SCREAMING_CASE constant, and `PersistentContext`
// ends in `Context` so it reads as a React context. Both are ordinary
// exported components (YapprLanding/components/{FAQ,PersistentContext}.tsx).
// Allowlisting them keeps their real export names instead of renaming the
// public API to satisfy a name-shape guess.
const COMPONENT_ALLOW = new Set(['FAQ', 'PersistentContext']);
export const isComponentName = (n) => COMPONENT_ALLOW.has(n) || (!n.endsWith('Props') && !/^[A-Z][A-Z0-9_]+$/.test(n)
