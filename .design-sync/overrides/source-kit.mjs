...
export async function resolvePackage(ctx) {
  ...
  if (srcRoot) {
    for (const c of components) {
      ...
      // FORK: upstream derives the group from the src path. Yappr's DS spans
      // two unrelated trees (YapprLanding/components/** and
      // src/renderer/shared/ui/**), so path derivation yields "yapprlanding" /
      // "shells" / "shared" — directory names, not design-system sections.
      // Every component here has a doc in .design-sync/docs/ carrying an
      // explicit `category:` (Landing / App Chrome / App UI), and
      // package-build only lets a doc category win when the group is still
      // 'general'. Leaving it at 'general' hands grouping to the docs, which
      // are the intentional source. JSDoc @category still wins where present.
